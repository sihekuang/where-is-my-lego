...
**IMPORTANT design note:** the `extract*` functions are **synchronous** — they call `translate(src)` and use the return value directly. The real translator (`makeTranslator`) is **async**. So you must NOT pass the async translator straight into the extractors (the cells would fill with unresolved `Promise`s and serialize as `{}`). Instead use a 3-phase approach: (1) run the extractors once with a *recording* sync translator to discover which source strings are stale (need translation); (2) `await` the async translator on each discovered string to build a `Map`; (3) run the extractors again with a *lookup* sync translator that reads the map. The extractors are pure and cheap, so running them twice is fine, and the manifest they produce is identical both times (it depends only on source hashes).

- [ ] **Step 1: Add structured loaders + sync runner + manifest harvester** (append to `translate.mjs`):
...
- [ ] **Step 2: Fold structured units into `currentManifest`.** Replace `currentManifest(sources)` so it also harvests structured hashes:
...
Update `runCheck` to build `const structured = loadStructured();` and call `currentManifest(sources, structured)`.

- [ ] **Step 3: Seed structured output in `runSeed` (3-phase async).** Inside the per-locale loop in `runSeed`, after the prose block, add:
