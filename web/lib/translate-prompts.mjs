// Shared translator prompts + system-prompt builder. Zero heavy deps (only the
// glossary), so it is safe to import from either backend. Both translator
// backends — translate-local.mjs (default) and translate-anthropic.mjs
// (fallback) — build their system prompt from here, so the instructions and the
// protected-terms glossary stay byte-for-byte identical across models.
...
//  - "ui": one short interface string with interpolation tokens to preserve.
...
// Compose the full system prompt for a (locale, mode): the mode rules with the
// language filled in, plus the shared protected-terms glossary so proper nouns
// (American Fork, AFPD, BAM, party names) and the case-sensitive status tokens
// survive every mode. Identical for every backend.
export function buildSystem(localeCode, mode = "document") {
  const rules = PROMPTS[mode] ?? PROMPTS.document;
  return `${rules.replace("{LANG}", LANG[localeCode] ?? localeCode)}\n\n${glossaryPromptBlock(localeCode)}`;
}
